#include "dingus/dxutils/D3DEnumeration.hpp"
...
// Returns number of depth bits for D3DFORMT.
...
SD3DDeviceInfo::~SD3DDeviceInfo()
{
	...
	// DINGUS_SAFE_RELEASE(configDB);  // TODO(kasicass): need it?
}
...
HRESULT CD3DEnumeration::enumerate()
{
	...
	for (int adOrd = 0; adOrd < adapterCount; ++adOrd)
	{
		...
		if (adapterInfo) return E_OUTOFMEMORY;
		...
		TDwordVector adapterFormatList;
		...
		for (int ibbf = 0; ibbf < bbufferFormatCount; ++ibbf)
		{
			...
			for (int iiw = 0; iiw < 2; ++iiw)
			{
				...
				if (FAILED(mDirect3D->CheckDeviceType(
					deviceInfo.caps.getAdapterOridinal(), deviceInfo.caps.getDeviceType(),
				...
				devCombo->adapterOrdinal = deviceInfo.caps.getAdapterOridinal();
