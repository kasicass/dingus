#include "dingus/renderer/RenderableBuffer.hpp"
...
void CRenderableBuffer::render(const CRenderContext& ctx)
{
	...
	// stats
	stats.incDrawCalls();
	stats.incVerticesRendered( getPrimCount() );
...
void CRenderableIndexedBuffer::render(const CRenderContext& ctx)
{
	if (getLargestActiveStream() < 0)
		return;
	if (getPrimCount() == 0)
	...
	dx.DrawIndexedPrimitive(getPrimType(), mBaseVertex, mMinVertex, mNumVertices, mStartIndex, getPrimCount());
	unapplyStreams();
