...
void CD3DDeviceCaps::setData(IDirect3D9& d3d, UINT adapterOrdinal, D3DDEVTYPE devType, const D3DCAPS9& caps)
{
	...
	if (r == ERROR_SUCCESS)
	{
		...
		r = RegQueryValueEx(hKey, "~MHz", NULL, NULL, (LPBYTE)&mCpuMhz, &size);
	}
	RegCloseKey(hKey);
	...
	if (vs >= D3DVS_VERSION(3, 0)) mVShaderVersion = VS_3_0;
	else if (vs >= D3DVS_VERSION(2, 0)) mVShaderVersion = VS_2_0;
	else if (vs >= D3DVS_VERSION(1, 1)) mVShaderVersion = VS_1_1;
	else mVShaderVersion = VS_FFP;

	// PS version
	DWORD ps = mCaps.PixelShaderVersion;
	if (ps >= D3DPS_VERSION(3,0)) mPShaderVersion = PS_3_0;
	else if (ps >= D3DPS_VERSION(2,0)) mPShaderVersion = PS_2_0;
	else if (ps >= D3DPS_VERSION(1,4)) mPShaderVersion = PS_1_4;
	else if (ps >= D3DPS_VERSION(1,1)) mPShaderVersion = PS_1_1;
	else mPShaderVersion = PS_TSS;
...
int CD3DDeviceCaps::calcVideoMemory()
{
	...
	for( int i = 0; i < context.mAdapterCount; ++i ) {
		if( !_stricmp( context.mAdapters[i].DriverName, mAdapterID.DeviceName ) )
