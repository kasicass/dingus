...
// NOTE: almost no fixed function T&L states are defnied here. Just don't use
...
SFxState FX_STATES[] = {
	...
	{ FXST_RENDERSTATE,    "PointScaleEanble",                D3DRS_POINTSCALEENABLE            },
	...
	{ FXST_RENDERSTATE,    "MultiSampleAnitAlias",            D3DRS_MULTISAMPLEANTIALIAS        },
...
int findState(const char* name)
{
	for (int i = 0; i < FX_STATES_SIZE; ++i)
	{
		if (0 == _stricmp(FX_STATES[i].name, name))
			return i;
	}
...
public:
	...
	void debugPrintFx(const char* fileName)
	{
		...
		for (size_t i = 0; i < n; ++i)
		{
			...
			if (s.stage < 0)
			{
				fprintf(f, "\t%s = %i\n", FX_STATES[s.index].name, s.value);
			}
			else
			{
				fprintf(f, "\t%s[%i] = %i\n", FX_STATES[s.index].name, s.stage, s.value);
			}
		}
		fprintf(f, "}\n");
	}
	...
	STDMETHOD(SetVertexShaderConstantI)(UINT registerIdx, const int* data, UINT registerCount)
	{
		return S_OK;   // don't inspect
	}

	STDMETHOD(SetVertexShaderConstantB)(UINT registerIdx, const BOOL* data, UINT registerCount)
	{
		return S_OK;   // don't inspect
	}
...
private:
	...
	std::vector<int> mPassStart;   // starting state index for each pass
	std::vector<int> mPassCounts;  // state count for each pass
...
private:
	...
	{
		SStateRequired(int idx) : index(idx)
		{
			assert(index >= 0 && index < FX_STATES_SIZE);
		}
	...
	{
		SStateDependent(int idx, DWORD val) : index(idx), value(val)
		{
			assert(index >= 0 && index < FX_STATES_SIZE);
		}
...
std::string CEffectRestorePassGenerator::checkEffect(const CEffectStateInspector& fx) const
{
	...
	// Check required states: the first pass must contain all required states
	...
	for (i = 0; i < mStatesDependent.size(); ++i)
	{
		...
		if (found)
		{
			...
			// all of them are present int the current pass.
			...
			for (size_t j = 0; i < depState.needed.size(); ++j)
			{
...
{
	DINGUS_SAFE_DELETE(gFxRestorePassGen);
}
	...
	CConsoleChannel& console)
{
	...
	// 5. supply restoring pass as macros; load effect again
	// 6. check that is has a restoring pass
	...
	{
		if (stateManager) fx->SetStateManager(stateManager);
	...
	*/
	...
	errorMsgs = "";
	assert(pool);
	hres = D3DXCreateEffectFromFileEx(
		&CD3DDevice::getInstance().getDevice(),
		fileName.c_str(), newMacros, NULL,
		skipConstants, optimizeShaders ? 0 : D3DXSHADER_SKIPOPTIMIZATION,
		pool, &fx, &errors);
	if (errors && errors->GetBufferSize() > 1)
	{
