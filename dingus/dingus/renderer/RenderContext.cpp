	...
	mDirectCurrFX(NULL)
{
	...
	mGlobalParams.addMatrix4x4("mView", mRenderCamera.getViewMatrix());
	mGlobalParams.addMatrix4x4("mProjection", mRenderCamera.getProjectionMatrix());
	mGlobalParams.addMatrix4x4("mViewProj", mRenderCamera.getViewProjMatrix());
...
struct SRenderableSorter
{
	...
	bool operator()(const CRenderable* ra, const CRenderable* rb)
	{
		...
		bool bback = ea->isBackToFrontSorted();
		if (aback && bback)
		{
			...
			if (aor && bor)
			{
				...
				// happend in Debug build... Spent 3 days on this! :(
...
void CRenderContext::renderRenderables()
{
	...
	//  3. vertex shader req. (in ascending order - nont, 1.1, 2.0 etc)
	...
	auto itR = mRenderables.begin();
	auto itEnd = mRenderables.end();
	auto itFxStart = itR;
	for (; itR != itEnd; ++itR)
	{
		...
		//
		// check effect chagne. if changed, render all renderables of the old effect
		...
		if (newFX != currFX)
		{
			...
			if (currFX)
			{
				...
				for (int p = 0; p < passes; ++p)
				{
					currFX->beginPass(p);
					// all objects
					for (auto it = itFxStart; it != itR; ++it)
					{
						CRenderable* r = *it;
						assert(r);
						r->beforeRender(*this);
